...
import lab4.Log;
import lab4.creatLogFromText;
import myException.graphExceptions;
import vertex.Vertex;

public class GraphPoetApp {
	
	static GraphPoet G = new GraphPoet();

	public static void main(String[] args) {
		GraphPoetApp f = new GraphPoetApp();
		JFrame frame = new JFrame("GraphPoetApp");
			...
			input.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (JFileChooser.APPROVE_OPTION == fc.showOpenDialog(new JFrame())) {
						...
						try {
							G = new GraphPoetFactory().createGraph(path);
			...
			addVertex.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (matcher.matches()) {
						...
						Vertex v = new WordVertexFactory().createVertex(vLabel, null);
			...
			addEdge.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent event) {
					...
					if (matcher.find()) {
						...
						if (!eType.equals("WordNeighborhoodEdge") || !YN.equals("Yes")) {
							JOptionPane.showMessageDialog(null, "Input invalid");
						}
						...
						if (se.size() < 2) {
							...
							Edge e = new WordNeighborhoodEdgeFactory().createEdge(eLabel, se, Double.valueOf(weight));
							G.addEdge(e);
							text.setText("");
						}
					} else {
	...
	class graphMetrics extends JPanel {
		public graphMetrics() {
			...
			degreeCentrality2input.setBounds(230, 100, 250, 40);
			...
			degreeCentrality2out.setBounds(500, 100, 80, 40);
			...
			closenessCentralityinput.setBounds(250, 180, 250, 40);
			...
			closenessCentralityout.setBounds(520, 180, 80, 40);
			...
			betweennessCentralityinput.setBounds(280, 260, 250, 40);
			...
			betweennessCentralityout.setBounds(550, 260, 80, 40);
			...
			inDegreeCentralityinput.setBounds(250, 340, 250, 40);
			...
			inDegreeCentralityout.setBounds(520, 340, 80, 40);
			...
			outDegreeCentralityinput.setBounds(260, 420, 250, 40);
			...
			outDegreeCentralityout.setBounds(530, 420, 80, 40);
			...
			distanceout.setBounds(740, 500, 80, 40);
	...
	class modifyEdge extends JPanel {
		public modifyEdge() {
			...
			text.setBounds(50, 300, 700, 80);
			text.setFont(new Font("宋体", Font.PLAIN, 24));
			JButton addEdge = new JButton("modifyEdge");
			addEdge.setBounds(790, 320, 100, 50);
			addEdge.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent event) {
					String input = text.getText();
					...
					if (matcher.find()) {
						...
						String YN = matcher.group(7);
						if (!eType.equals("WordNeighborhoodEdge") || !YN.equals("Yes")) {
							JOptionPane.showMessageDialog(null, "Input invalid1");
						}
						List<Vertex> se = new ArrayList<Vertex>();
						for (Vertex v : G.vertices()) {
							if (v.getLabel().equals(start)) {
						...
						if (se.size() < 2) {
							JOptionPane.showMessageDialog(null, "No enough vertices" + se.size());
						} else {
							Edge e = new WordNeighborhoodEdgeFactory().createEdge(eLabel, se, Double.valueOf(weight));
