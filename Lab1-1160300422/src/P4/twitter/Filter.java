...
public class Filter {
	...
	public static List<Tweet> writtenBy(List<Tweet> tweets, String username) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (now.getAuthor().equalsIgnoreCase(username)) {
				tweetsFromUsername.add(now);
			}
		}
	...
	public static List<Tweet> inTimespan(List<Tweet> tweets, Timespan timespan) {
		...
		for (int i = 0; i < n; i++) {
			Tweet now = tweets.get(i);
			if (now.getTimestamp().isAfter(start) && now.getTimestamp().isBefore(end)) {
				tweetsInTimespan.add(now);
			}
		}
	...
	public static List<Tweet> containing(List<Tweet> tweets, List<String> words) {
		List<Tweet> tweetsContainWords = new ArrayList<Tweet>();
		int m = words.size();
		int n = tweets.size();
		for (int i = 0; i < n; i++) {
			Tweet now = tweets.get(i);
			String[] splitText = now.getText().split(" ");
			int length = splitText.length;
			boolean choice = false;
			for (int j = 0; j < length; j++) {
				for (int k = 0; k < m; k++) {
					if (splitText[j].equalsIgnoreCase(words.get(k))) {
						tweetsContainWords.add(now);
						choice = true;
						break;
					}
				}
